...
export default function AddLanguages() {
  ...
  let sendingLanguagesDataToApi = async () => {
    await axios.post(`http://127.0.0.1:8000/api/languages`, languages).then((res) => {
  ...
  let submitLanguagesForm = async (e) => {
    e.preventDefault();
    let validation = validateLanguagesForm();
    if (!validation?.error) {
      sendingLanguagesDataToApi();
  ...
  return (
    <>
      <form onSubmit={submitLanguagesForm}>
        ...
        <button type='submit' className="w-full py-2  text-white  bg-gray-600 rounded-md ">Add Language</button>
      </form>
    </>
  )
}
